...
class noaa:
    ...
    def clear(self):
        ...
        for count in range (0, MAXDAYS+1):
            ...
            set_property('Day%i.FanartCode'    % count, '0')
    ...
    def fetchDaily(self,num):
        ...
        for count, item in enumerate(data['periods'], start=0):
            ...
            if rain and str(rain) and not "0" == str(rain):
                ...
                clear_property('Daily.%i.Precipitation'    % (count+1))
    ...
    def fetchCurrent(self,num):
        ...
        try:
            feelslike=FEELS_LIKE_C_KPH(data.get('temperature').get('value'), windspeed, data.get('relativeHumidity').get('value'))
            if feelslike:
                #xbmc.log('feelslike: %s' % (feelslike),level=xbmc.LOGERROR)
        ...
        if data.get('windChill').get('value'):
            ...
            if 'F' in TEMPUNIT:
                set_property('Current.WindChill', '%s%s' % (int(round(CtoF(data.get('windChill').get('value')))), TEMPUNIT))
            elif 'C' in TEMPUNIT:
                set_property('Current.WindChill', '%s%s' % (int(round(data.get('windChill').get('value'))), TEMPUNIT))
            #feels like wants raw celcius value
        ...
        if data.get('heatIndex').get('value'):
            ...
            if 'F' in TEMPUNIT:
                set_property('Current.heatIndex', '%s%s' % (int(round(CtoF(data.get('heatIndex').get('value')))), TEMPUNIT))
            elif 'C' in TEMPUNIT:
                set_property('Current.heatIndex', '%s%s' % (int(round(data.get('heatIndex').get('value'))), TEMPUNIT))
            #feels like wants raw celcius value
    ...
    # extended properties
        for count, item in enumerate(data['periods'], start = 0):
            ...
            if rain and str(rain) and not "0" == str(rain):
                ...
                clear_property('Hourly.%i.Precipitation'    % (count+1))
