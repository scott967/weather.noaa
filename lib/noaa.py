...
class Noaa:
    ...
    def fetchDaily(self,num):
        ...
        if daily_weather and 'properties' in daily_weather:
            ...
            xbmc.log(f'failed to find weather data from : {url}', level=xbmc.LOGERROR)
        ...
        for count, item in enumerate(data['periods'], start=0):
            ...
            if item['isDaytime']:
                ...
                if 'F' in TEMPUNIT:
                    set_property(f'Day{count}.HighTemp', str(int(round(FtoC(item['temperature'])))))
                    set_property(f'Day{count}.LowTemp', str(int(round(FtoC(item['temperature'])))))
                elif 'C' in TEMPUNIT:
                    set_property(f'Day{count}.HighTemp', str(int(round(item['temperature']))))
                    set_property(f'Day{count}.LowTemp', str(int(round(item['temperature']))))
            ...
            set_property(f'Daily.{count+1}.ShortOutlook', item['shortForecast'])
            set_property(f'Daily.{count+1}.DetailedOutlook', item['detailedForecast'])

            set_property(f'Daily.{count+1}.RemoteIcon', icon)
            set_property(f'Daily.{count+1}.OutlookIcon', WEATHER_ICON % weathercode)
            set_property(f'Daily.{count+1}.FanartCode', weathercode)
            set_property(f'Daily.{count+1}.WindDirection', item['windDirection'])
            set_property(f'Daily.{count+1}.WindSpeed', item['windSpeed'])

            if item['isDaytime']:
                set_property(f'Daily.{count+1}.LongDay', item['name'])
                set_property(f'Daily.{count+1}.ShortDay', get_weekday(startstamp,'s')+" (d)")
                ...
                set_property(f'Daily.{count+1}.TempDay', '%s%s' % (item['temperature'], TEMPUNIT))
                set_property(f'Daily.{count+1}.HighTemperature', f'{item['temperature']}{TEMPUNIT}')
                set_property(f'Daily.{count+1}.TempNight', '')
                set_property(f'Daily.{count+1}.LowTemperature', '')

            if not item['isDaytime']:
                set_property(f'Daily.{count+1}.LongDay', item['name'])
                set_property(f'Daily.{count+1}.ShortDay', get_weekday(startstamp,'s')+" (n)")

                set_property(f'Daily.{count+1}.TempDay', '')
                set_property(f'Daily.{count+1}.HighTemperature', '')
                ## we passed units to api, so we got back C or F, so don't need to convert
                set_property(f'Daily.{count+1}.TempNight', '%s%s' % (item['temperature'], TEMPUNIT))
                set_property(f'Daily.{count+1}.LowTemperature',
                             '%s%s' % (item['temperature'],
                            TEMPUNIT))

            if [1] == 'd' or DATEFORMAT[0] == 'D':
                set_property(f'Daily.{count+1}.LongDate', get_datestr(startstamp, 'dl'))
                set_property(f'Daily.{count+1}.ShortDate', get_datestr(startstamp, 'ds'))
            else:
                set_property(f'Daily.{count+1}.LongDate', get_datestr(startstamp, 'ml'))
                set_property(f'Daily.{count+1}.ShortDate', get_datestr(startstamp, 'ms'))
    ...
    def fetchAltDaily(self,num):
        ...
        if daily_weather and 'data' in daily_weather:
            ...
            xbmc.log(f'failed to retrieve weather data from : {url}', level=xbmc.LOGERROR)
            xbmc.log(f'{daily_weather}', level=xbmc.LOGERROR)
            return None
        ...
        for count, item in enumerate(dailydata, start=0):
            ...
            if item['tempLabel'] == 'High':
                set_property(f'Daily.{count+1}.LongDay', item['startPeriodName'])
                set_property(f'Daily.{count+1}.ShortDay', get_weekday(startstamp,'s')+" (d)")
        ...
        if daily_weather and 'currentobservation' in daily_weather:
            ...
            set_property('Current.OutlookIcon', f'{weathercode}.png') # xbmc translates it to Current.ConditionIcon
    ...
    def fetchCurrent(self,num):
        ...
        parsed_metar = Metar(data['rawMessage']).string() if data.get('rawMessage') else ''
        ...
        if icon:
            ...
            set_property('Current.OutlookIcon',
                         f'{weathercode}.png') # xbmc translates it to Current.ConditionIcon
            set_property('Current.FanartCode', weathercode)

        set_property('Current.Condition', FORECAST.get(data.get('textDescription',''), ''))
        try:
    ...
    def fetchWeatherAlerts(self,num):
        ...
        if alerts and 'features' in alerts:
            ...
            xbmc.log(f'failed to get proper alert response {url}', level=xbmc.LOGERROR)
        ...
        if 'features' in alerts and alerts['features']:
            ...
            xbmc.log(f'No current weather alerts from {url}', level=xbmc.LOGDEBUG)
            return
        ...
        for count, item in enumerate(data, start=1):
            ...
            set_property(f'Alerts.{count}.messageType', str(thisdata['messageType']))
            set_property(f'Alerts.{count}.category', str(thisdata['category']))
            set_property(f'Alerts.{count}.severity', str(thisdata['severity']))
            set_property(f'Alerts.{count}.certainty', str(thisdata['certainty']))
            ...
            set_property(f'Alerts.{count}.description', str(thisdata['description']))
            set_property(f'Alerts.{count}.instruction', str(thisdata['instruction']))
    ...
    def fetchHourly(self,num:str):
        ...
        if hourly_weather and 'properties' in hourly_weather:
            ...
            xbmc.log(f'failed to find proper hourly weather from {url}', level=xbmc.LOGERROR)
            return
    ...
    # extended properties
        for count, item in enumerate(data['periods'], start = 0):
            ...
            startstamp = get_timestamp(starttime)
            if DATEFORMAT[1] == 'd' or DATEFORMAT[0] == 'D':
                set_property(f'Hourly.{count+1}.LongDate', get_fulldatestr(startstamp, 'dl'))
                set_property(f'Hourly.{count+1}.ShortDate', get_fulldatestr(startstamp, 'ds'))
            else:
                set_property(f'Hourly.{count+1}.LongDate', get_fulldatestr(startstamp, 'ml'))
                set_property(f'Hourly.{count+1}.ShortDate', get_fulldatestr(startstamp, 'ms'))
            ...
            set_property(f'Hourly.{count+1}.ShortOutlook',
                         FORECAST.get(item['shortForecast'], ''))
            set_property(f'Hourly.{count+1}.OutlookIcon', WEATHER_ICON % weathercode)
            ...
            if windspeed and item['windDirection']:
                set_property(f'Hourly.{count+1}.WindDirection', item['windDirection'])
            ...
            ## we passed units to api, so we got back C or F, so don't need to convert
            set_property(f'Hourly.{count+1}.Temperature', f'{int(round(item['temperature']))}{TEMPUNIT}')
            ##if 'F' in TEMPUNIT:
            ...
            if rain and str(rain) and "0" != str(rain):
                set_property(f'Hourly.{count+1}.Precipitation', str(rain) + '%')
            else:
    ...
    def __init__(self):
        ...
        if sys.argv[1].startswith('FetchLocation'):
            ...
            try:
                lastPointsCheck = ADDON.getSetting(f'Location{num}lastPointsCheck')
            ...
            if LatLong:
                ...
                if (radarLoop == "true"):
                    ...
                    loop_image = get_url_image(url, dest) if get_url_image(url, dest) else ''
